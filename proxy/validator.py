...
from config.configfile import VALIDATOR_BASE_URL, VALIDATOR_BATCH_COUNT, HEADERS, REQUEST_TIMEOUT
...
class Validator:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.get(
                    VALIDATOR_BASE_URL, headers=HEADERS, proxy=proxy, timeout=REQUEST_TIMEOUT
                ) as resp:
                    if resp.status == 200:
                        self.redis.increase_proxy_score(proxy)
                        proxy_logger.info("Validator √ {}".format(proxy))
                    else:
    ...
    def run(self):
        ...
        启动校验器
        """
        proxy_logger.info("Validator working...")
        proxy_logger.info("Validator website is {}".format(VALIDATOR_BASE_URL))
        ...
        proxy_logger.info("Validator resting...")
